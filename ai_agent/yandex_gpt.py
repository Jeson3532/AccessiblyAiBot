...
def generate_task(profile_comp: str, material: str, temp=0.5):
    ...
    ai_answer = model.run(
        ...
        f"Уровень сложности - в зависимости от уровня знаний в той или иной области студента. Предоставь также эталонное решение для проверки. Ответ пришли в формате 'Вопросы для студента' жирным шрифтом, и 'Ответы на вопросы:' жирным шрифтом (можно добавить смайлики строго по тематике в заголовки). Все должно быть оформлено красиво и лаконично. ФОРМАТИРОВАНИЕ СТРОГО В HTML, ИСПОЛЬЗУЯ ТОЛЬКО ТЕГ <b>. НЕ УКАЗЫВАЙ В НАЧАЛЕ ``` И НЕ ПИШИ ЧТО ЗА ТИП ДОКУМЕНТА")
    return preprocess_text(ai_answer.alternatives[0].text)
